node("master") {
    ...
    // Run the test
    sh "${mvnHome}/bin/mvn clean install perfana-gatling:test -Ptest-env-local-via-jenkins,test-type-load,assert-results -DtestRunId=$testRunId -DbuildResultsUrl=$buildUrl"
